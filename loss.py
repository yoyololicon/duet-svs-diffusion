...
def diffusion_elbo(gamma_0, gamma_1, d_gamma_t, x, noise, noise_hat):
    gamma_0 = gamma_0.float()
    gamma_1 = gamma_1.float()
    d_gamma_t = d_gamma_t.float()
    x = x.float()
    noise = noise.float()
    ...
    prior_loss = 0.5 * (
        log_var_1.exp() + x_dot * torch.exp(log_alpha_1 * 2) - 1 - log_var_1
    )
    ...
    extra_dict = {"kld": prior_loss.item(), "ll": ll.item()}
    # diffusion loss
    diff = noise - noise_hat
    loss_T_raw = 0.5 * (d_gamma_t * (diff * diff).mean(1)) / x.shape[0]
